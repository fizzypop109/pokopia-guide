...
export async function generateMetadata({
  ...
  return {
    ...
    description: `${p.classification}. Specialty: ${p.specialty}. Habitat: ${p.idealHabitat}.`,
  };
}
  ...
  return (
    <div className="mx-auto max-w-4xl px-6 py-10 flex flex-col gap-8">
      ...
      <header className="flex flex-col sm:flex-row sm:items-center gap-6">
        <div className="shrink-0 w-48 h-48 rounded-2xl bg-linear-to-br from-emerald-50 to-zinc-100 dark:from-emerald-950/40 dark:to-zinc-900 flex items-center justify-center p-4">
          <Image
        ...
        <div className="flex flex-col gap-3 min-w-0">
          <div className="flex items-center gap-3 text-sm">
            ...
            <span
              className={`px-2 py-0.5 rounded-full font-medium ${RARITY_COLOR[p.rarity]}`}
            >
              {p.rarity}
            </span>
          </div>
      ...
      <Section title="Local Habitats" hint="Specific spots where they nest">
          <div className="flex flex-row gap-3">
              {p.localHabitats.map(h => {
                  const habitatSlug = h.toLowerCase().replace(/\s/g, "-");
                  return (
                      <Card key={h} href={`/habitats/${habitatSlug}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Section title="Available Times">
          <Chips items={p.availableTimes} />
        </Section>

        <Section title="Available Weather">
          <Chips items={p.availableWeather} />
        </Section>
      </div>
